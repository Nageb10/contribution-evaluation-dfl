...
noisy=client5"
      python3 -m fltabular.run_gossip \
        ...
        --seed $REP \
        --noisy-client 4 \
        >> "$LOG_DIR/noisy_${TOPO}_iid_${METHOD}.log" 2>&1
    done
...
method=$METHOD rep=$REP noisy=client5"
        python3 -m fltabular.run_gossip \
          --topology $TOPO \
          --partition dirichlet \
          --alpha $ALPHA \
          ...
          --seed $REP \
          --noisy-client 4 \
          >> "$LOG_DIR/noisy_${TOPO}_dirichlet${ALPHA}_${METHOD}.log" 2>&1
      done
    done
...
echo "All noisy experiments complete."
